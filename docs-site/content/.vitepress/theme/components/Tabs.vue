...
const augmentedTabs = computed(() =>
  // We don't want "Other Languages" to show up for e.g. Sample Response tabs.
...
// Remember the currently shown tab so we don't snap back to the first tab when
// the language override drops (picker cleared or expanded to >1 language).
...
<!--
  Only the styling below targets the Shiki-generated `.language-*` / `<pre>`
  markup, which we can't attach Tailwind classes to. Everything on our own
  markup above uses Tailwind utilities.
-->
<style scoped>
