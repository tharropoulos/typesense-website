...
// Rendered after every H2–H6 by the markdown pipeline. Extracts just this
// heading's section from the page markdown (provided by embed-markdown).
...
// Extract the markdown from this heading down to the next heading of the same
// or higher level, skipping fenced code blocks so nested `#` don't confuse us.
...
<!-- Reveal-on-hover targets the VitePress-generated parent heading, so it must
     be CSS (we can't put a Tailwind `group` class on that heading). -->
<style scoped>
