...
And then define the [Collection Schema](/30.2/api/collections.html#with-pre-defined-schema) in the `config/scout.php` file:
...
You can also set [search parameters](/30.2/api/documents.html#search-parameters) for searching through your collections on the fly. For example, you can set the `query_by` parameter to search by a game's Steam App ID only:
