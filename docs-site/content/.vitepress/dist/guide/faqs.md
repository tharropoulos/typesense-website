...
In the field definition, in the collection schema, add the `infix: true` parameter documented [here](/30.2/api/collections.html#field-parameters), and then use the `infix` search parameter documented [here](/30.2/api/search.html#query-parameters) at search time.
...
You can enable stemming by setting the [`stem: true`](/30.2/api/collections.html#field-parameters) parameter in the field definition in the collection schema.
...
For more details on stemming, read the [stemming documentation](/30.2/api/stemming).
...
But for performance reasons, Typesense will only consider the top `4` prefixes and typo variations by default. The `4` is configurable using the `max_candidates` search parameter, documented [here](/30.2/api/search.html#ranking-and-sorting-parameters).

In short, if you search for a short term like say "a", and not all the records you expect are returned, you want to increase `max_candidates` to a higher value and/or change the [`default_sorting_field`](/30.2/api/collections.html#schema-parameters) in the collection schema to define "top" using some popularity score in your records, without which Typesense will just use the count of each records for each prefix to define top.
...
2. By default, when no results are found for a multi-keyword search query, then Typesense will start dropping words in the query one at a time, and repeat the search until enough results are found.
   ...
   The definition of "enough" above can be configured using the [`drop_tokens_threshold`](/30.2/api/search.html#search-parameters) search parameter and the direction of the dropping (left to right vs right to left) can be configured using the  [`drop_tokens_mode`](/30.2/api/search.html#search-parameters) parameter.
...
Typesense has a [search parameter](/30.2/api/search.html#search-parameters) called `q` 
...
If you need to do more structured queries, then you can use the [`filter_by` search parameter](/30.2/api/search.html#filter-parameters), 
...
This value of 4 is configurable using the `max_candidates` search parameter, documented [here](/30.2/api/search.html#ranking-and-sorting-parameters).
...
So if you find that you're getting too many irrelevant results with semantic search, you can use the `distance_threshold` parameter in `vector_query` to pick an appropriate threshold that works for your use case. Read more about this [here](/30.2/api/vector-search.html#distance-threshold).

If you're using Hybrid Search, you can control the weightage of keyword vs semantic matches using the `alpha` parameter of `vector_query`. Read more about this [here](/30.2/api/vector-search.html#weightage-for-semantic-vs-keyword-matches).
...
You'd want to do this 2nd query inside a [`multi_search`](/30.2/api/federated-multi-search.html) request, so it's still a single http request.
...
If you're using the [`import`](/30.2/api/documents.html#index-multiple-documents) API endpoint to bulk import documents into Typesense, 
...
You can use the [`?return_id=true`](/30.2/api/documents.html#returning-the-id-of-the-imported-documents) query parameter to get Typesense to return the exact document ID that errored out.
...
You can access these metrics using the [`GET /metrics.json`](/30.2/api/cluster-operations.html#cluster-metrics) endpoint.

### How do I set up HTTPS with Typesense?

By default, Typesense runs on port `8108` and serves HTTP traffic.

To enable HTTPS, you want to change the [`api-port`](/30.2/api/server-configuration.html#networking) to `443` and then use the [`ssl-certificate` and `ssl-certificate-key`](/30.2/api/server-configuration.html#ssl-https) parameters to point to your SSL certificate and SSL private key respectively.
