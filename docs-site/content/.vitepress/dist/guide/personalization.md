...
Typesense's [Vector Query Re-Ranking feature](/30.2/api/vector-search.html#rank-keyword-search-via-vector-search) allows you to bring [embeddings](/30.2/api/vector-search.html#what-is-an-embedding) from an ML model (that takes users' past behaviors as input), 
