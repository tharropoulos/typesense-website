...
You can set non-common fields/attributes as optional in the [collection schema](/30.2/api/collections.html#with-pre-defined-schema).
...
You can then generate individual [Scoped Search API Keys](/30.2/api/api-keys.html#generate-scoped-search-key) for each user and restrict that key to only be able to access records that have this user's ID in the `friends_with_user_ids` attribute.
