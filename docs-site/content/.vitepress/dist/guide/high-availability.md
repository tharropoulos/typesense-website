...
`peering_address`, `peering_port` and `api_port` should match the corresponding [Server Configuration Parameters](/30.2/api/server-configuration.html) used when starting the Typesense process on each node.
...
3. Trigger a snapshot on the leader using the [`/operations/snapshot` endpoint](/30.2/api/cluster-operations.html#create-snapshot-for-backups). (You can leave out the ?path query parameter so the snapshot is generated internally, but not exported out of Typesense). This will compact all the writes in the Raft logs into the snapshot.
