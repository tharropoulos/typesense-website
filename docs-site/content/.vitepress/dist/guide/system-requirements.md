...
When indexing documents for [Vector Search](/30.2/api/vector-search.html), each vector requires 7 bytes of memory. 
...
When using [built-in embedding models](/30.2/api/vector-search.html#using-built-in-models) for Semantic Search / Hybrid Search,
