...
You can do this by setting [`token_separators`](/30.2/api/collections.html#schema-parameters) in the schema when [creating the collection](/30.2/api/collections.html#create-a-collection):
...
We first want to tell Typesense to split by by `(`, `)` and `-` using the [`token_separators`](/30.2/api/collections.html#schema-parameters) setting in the schema when [creating the collection](/30.2/api/collections.html#create-a-collection):
...
To solve for the remaining cases above, we can use the [`token_separators`](/30.2/api/collections.html#schema-parameters) setting in the schema when [creating the collection](/30.2/api/collections.html#create-a-collection): 
...
Typesense does not have a native date/time [data type](/30.2/api/collections.html#field-types).

So you would have to convert dates and times to Unix Timestamps as described [here](/30.2/api/collections.html#indexing-dates).
...
To enable nested fields, you'll need to use the `enable_nested_fields` property when creating the collection, along with the `object` or `object[]` [data type](/30.2/api/collections.html#field-types):
...
Read more [here](/30.2/api/collections.html#indexing-nested-fields).
...
Here's more information about GeoSearch queries: [GeoSearch API Reference](/30.2/api/documents.html#geosearch).
...
please open a [GitHub issue](https://github.com/typesense/typesense/issues) or ask in our [Slack Community](https://typesense.link/slack-community). 
