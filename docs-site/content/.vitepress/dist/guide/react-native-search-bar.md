...
Typesense needs you to create a [collection](/30.2/api/collections.html) in order to search through documents. A collection is a named container that defines a schema and stores indexed documents for search. Collection bundles three things together:
...
3. Create the environment configuration file `.env`:
   ...
   Using env variables is **NOT recommended** for production apps since they are bundled with the app. Instead, you want to generate a [search-only API key](/30.2/api/api-keys.html#search-only-api-key) and serve it to your app from a backend endpoint, along with your Typesense cluster endpoints. This way you can rotate your API keys and change your cluster endpoints at any point as needed.
