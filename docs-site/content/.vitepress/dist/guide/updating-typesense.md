...
You can determine whether a node is a leader or follower by the value of the `state` field in the [`/debug`](/30.2/api/cluster-operations.html#debug) end-point response.
...
1. Trigger a snapshot to [create a backup](/30.2/api/cluster-operations.html#create-snapshot-for-backups) of your data on the leader node.
