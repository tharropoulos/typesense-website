...
1. Add your placeholder characters to the [`symbols_to_index`](/30.2/api/collections.html#schema-parameters) parameter in the collection schema.
...
```
For information on filtering `geopoint` fields to search within a specific area, please refer to our [documentation](/30.2/api/geosearch.html#searching-within-a-radius) on Geosearch.

## Filtering Nested Object Fields

You can also filter on [nested object fields](/30.2/api/collections.html#indexing-nested-fields) by using the dot (`.`) notation to refer to nested fields.
...
Filtering on joined fields is supported at any level of nesting. For a detailed explanation and examples, please refer to our [documentation on nested joins](/30.2/api/joins.html#nested-joins).

:::warning
Joins must be configured in the schema prior to using them in a filter.
:::

See [JOINs](/30.2/api/joins.html) for more details.
