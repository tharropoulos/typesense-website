...
4. Make a [Bulk Import API call](/30.2/api/documents.html#index-multiple-documents) to Typesense with just the records in Step 3, with `action=upsert`
5. For records that were marked as deleted in Step 2, make a [Delete by Query](/30.2/api/documents.html#delete-by-query) API call to Typesense with all the record IDs in a filter like this: `filter_by:=[id1,id2,id3]`.
...
2. Every say 5s, have a scheduled job that reads all the changes from this queue and [bulk imports](/30.2/api/documents.html#index-multiple-documents) them into Typesense.
...
you can also use the [Single Document Indexing API](/30.2/api/documents.html#index-a-single-document).
...
If your application generates more than 10s of writes-per-second, you want to switch to using the [Bulk Import API](/30.2/api/documents.html#index-multiple-documents), which is much more performant in handling high volume writes than the single document write endpoint.
...
You could use the [Collection Alias](/30.2/api/collection-alias.html) feature to do a reindex into a new collection and then switch the alias over to the new collection.
Or you can use the [Bulk Import API](/30.2/api/documents.html#index-multiple-documents) with `action=upsert`, along with the [Delete by Query](/30.2/api/documents.html#delete-by-query) endpoint on an existing collection and reimport your existing dataset.
...
7. You could also change the value of `healthy-read-lag` and `healthy-write-lag` [server configuration parameters](/30.2/api/server-configuration.html).
...
In the import API call, you'll notice a [parameter called `batch_size`](/30.2/api/documents.html#configure-batch-size).
