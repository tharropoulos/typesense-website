...
This is very similar to creating a regular collection, except for the addition of an [auto-embedding field](/30.2/api/vector-search.html#option-b-auto-embedding-generation-within-typesense) highlighted below.
...
You can also use
[OpenAI](/30.2/api/vector-search.html#using-openai-api), 
[PaLM API](/30.2/api/vector-search.html#using-google-palm-api) or
[Vertex AI API](/30.2/api/vector-search.html#using-gcp-vertex-ai-api) to have Typesense automatically make API calls out to these services for generating embeddings that will power the semantic search.
...
For more details on the `vector_query` parameter and its options, please refer to the [API documentation](/30.2/api/vector-search.html#nearest-neighbor-vector-search) for Vector Search.
...
Read the full API reference documentation about [Vector Search here](/30.2/api/vector-search.html).  
...
If you want to speed this process up, you want to enable [GPU Acceleration](/30.2/api/vector-search.html#using-a-gpu-optional) in Typesense.
