...
When you send documents to Typesense, it stores your data in the directory indicated by the `data-dir` [server configuration parameter](/30.2/api/server-configuration) and then builds the in-memory data structures that power search.
...
1. Call the [Snapshot API endpoint](/30.2/api/cluster-operations.html#create-snapshot-for-backups), specifying the directory to which to write the snapshot to on the server. 
