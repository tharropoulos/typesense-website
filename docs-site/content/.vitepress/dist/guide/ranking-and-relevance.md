...
2) calculated using a [counter analytics rule in Typesense](/30.2/api/analytics-query-suggestions.html#counting-events-for-popularity) 
...
However, there might be cases where this might not be desirable behavior. You can turn this off by setting `prioritize_exact_match=false` when [searching](/30.2/api/search.html#search-parameters).
...
The syntax for the expression inside `_eval()` is the same as the [`filter_by` search parameter](/30.2/api/search.html#filter-parameters), so we also call this feature "Optional Filtering".
...
1. Based on a search query by setting up [Overrides (aka Curation)](/30.2/api/curation.html) or
2. Dynamically using the [`pinned_hits` and `hidden_hits`](/30.2/api/search.html#ranking-and-sorting-parameters) search parameter
...
To turn off typo tolerance completely, set `num_typos=0` and `typo_tokens_threshold=0` when [searching](/30.2/api/search.html#typo-tolerance-parameters).

You can also tweak typo tolerance sensitivity by setting those values to higher numbers as needed.
To control typo tolerance based on word length, you can use the `min_len_1typo` and `min_len_2typo` [search parameters](/30.2/api/search.html#typo-tolerance-parameters).
