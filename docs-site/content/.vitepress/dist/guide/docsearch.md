...
[`embedding`](/30.2/api/vector-search.html#what-is-an-embedding) field
...
The example above uses one of the built-in ML models in Typesense, but you can use OpenAI, PaLM API or any other built-in ML model as described [here](/30.2/api/vector-search.html#option-b-auto-embedding-generation-within-typesense).
:::
