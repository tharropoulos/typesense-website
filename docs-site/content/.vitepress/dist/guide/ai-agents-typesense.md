...
**What Typesense does**: Combines both in a single query using [hybrid search](/30.2/api/vector-search.html#hybrid-search). You control the balance:
...
**What Typesense does**: Generates [embeddings server-side automatically](/30.2/api/vector-search.html#option-b-auto-embedding-generation-within-typesense):
...
**What Typesense does**: Built-in [conversational search](/30.2/api/conversational-search-rag.html#start-a-conversation) handles the entire RAG flow:
...
**[Multi-search](/30.2/api/federated-multi-search.html#federated-search) pattern**: Query multiple collections in a single API call:
...
See the [API Clients](/30.2/api/api-clients.html#libraries) reference for detailed usage and configuration.
...
2. Explore the [Vector Search](/30.2/api/vector-search.html#hybrid-search) API reference for hybrid search configuration.
3. Explore [Conversational Search](/30.2/api/conversational-search-rag.html) for RAG capabilities.
4. Browse the [full API reference](/30.2/api/) for all available features.
